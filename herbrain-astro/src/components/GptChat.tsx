  ...
  return (
    <div className="premium-card-static overflow-hidden">
      ...
      <button
        ...
        className="w-full flex items-center justify-between p-4 hover:bg-herbrain-surface/50 transition-colors"
      >
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-herbrain-green to-herbrain-green-dark flex items-center justify-center flex-shrink-0">
            <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 00-2.456 2.456z" />
            </svg>
          </div>
          <div className="text-left">
            <h3 className="text-[13px] font-semibold text-herbrain-dark">
              AI Neurobot
            </h3>
            <p className="text-[10px] text-herbrain-muted">
              Ask about brain changes during pregnancy
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <svg 
            className={`w-4 h-4 text-herbrain-muted transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`} 
            fill="none" 
            viewBox="0 0 24 24" 
            stroke="currentColor"
          >
      ...
      <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isExpanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-4 pb-4 border-t border-herbrain-border/30">
          {/* Settings Toggle */}
          <div className="flex justify-end pt-2 pb-2">
            <button
              ...
              title="Settings"
            >
              <svg className="w-3.5 h-3.5 text-herbrain-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {/* Settings Panel */}
          {showSettings && (
            <div className="mb-3 p-3 bg-herbrain-surface/70 rounded-xl">
              <p className="text-[10px] text-herbrain-muted mb-2">
              ...
              {apiKey ? (
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-green-500"></span>
                    <span className="text-[10px] text-herbrain-dark">Configured</span>
                  </div>
                  <button
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="sk-..."
                    className="premium-input flex-1 text-[11px] py-2"
                  />
                  <button onClick={saveApiKey} className="premium-btn text-[10px] py-2">
                    Save
                  </button>
                </div>
              )}
          ...
          <div
            ...
            className="h-32 overflow-y-auto rounded-xl bg-herbrain-surface/40 p-2.5 mb-3 custom-scrollbar"
          >
            {messages.length === 0 ? (
              <div className="h-full flex items-center justify-center">
                <p className="text-[10px] text-herbrain-muted/70 text-center">
                  {apiKey
          ...
          <form onSubmit={handleSubmit} className="flex gap-2">
            <input
              ...
              className="premium-input flex-1 text-[12px] py-2"
              onKeyDown={(e) => {
          ...
          {/* Disclaimer */}
          <p className="text-[9px] text-herbrain-muted/50 mt-2 text-center">
            Educational only. May be inaccurate. Not for medical use.
          </p>
        </div>
      </div>
    </div>
