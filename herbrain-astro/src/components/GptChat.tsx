  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-semibold text-herbrain-dark">
            Ask the AI Neurobot
          </h3>
        ...
        <button
          ...
          className="p-2 text-herbrain-muted hover:text-herbrain-dark transition-colors"
      ...
      {showSettings && (
        <div className="mb-4 p-4 bg-gray-50 rounded-lg">
          <p className="text-sm text-herbrain-muted mb-2">
            Enter your OpenAI API key to enable the chat feature.
          ...
          {apiKey ? (
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="sk-..."
                className="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-herbrain-green"
              />
              <button
                onClick={saveApiKey}
                className="px-4 py-2 text-sm bg-herbrain-green text-white rounded-lg hover:bg-herbrain-green/90"
              >
                Save
              </button>
            </div>
      ...
      <div
        ...
        className="h-48 overflow-y-auto border border-gray-200 rounded-lg p-3 mb-4 bg-gray-50"
      >
        {messages.length === 0 ? (
          <p className="text-sm text-herbrain-muted text-center py-8">
            {apiKey
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        <textarea
          ...
          rows={2}
          className="flex-1 px-3 py-2 border border-gray-300 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-herbrain-green disabled:bg-gray-100"
          onKeyDown={(e) => {
        ...
        <button
          ...
          className="px-4 py-2 bg-herbrain-green text-white rounded-lg hover:bg-herbrain-green/90 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Ask
        </button>
      </form>
      ...
      {/* Disclaimer */}
      <p className="text-xs text-herbrain-muted mt-3">
        Disclaimer: Neurobot is an educational tool. It may generate incomplete or
        inaccurate responses and should not be used for medical purposes.
      </p>
    </div>
  );
}
